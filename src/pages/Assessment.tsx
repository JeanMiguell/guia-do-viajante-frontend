      ...
      <div className="flex-1 md:ml-64 flex flex-col md:flex-row gap-6 md:gap-8 px-4 md:px-10 py-6 md:py-10 pb-24 md:pb-10 items-start">
        ...
        <div className="flex-1 min-w-0 w-full order-last md:order-first">
          ...
          <div className="mb-6 md:mb-8">
            ...
            <p className="text-gray-500 mt-1 text-sm md:text-base">Teste seus conhecimentos sobre os eventos da linha do tempo.</p>
          </div>
                ...
                {assessments.map((a, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`
                        flex items-center gap-3 md:gap-5 p-4 md:p-5 rounded-3xl border transition-all cursor-pointer
                        ${locked && !done
                      ...
                      `}
                    >
                      <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl overflow-hidden flex-shrink-0 flex items-center justify-center border ${done ? "bg-[#e8f5e8] border-[#b8d9b8]" : "bg-[#f4ead7] border-[#e8dfcf]"}`}>
                        {a.imageUrl && !locked ? (
                          <img src={a.imageUrl} alt={a.activityName} className="w-full h-full object-cover" />
                        ) : locked && !done ? (
                          <Lock size={18} className="text-[#b0a080]" />
                        ) : done ? (
                          <span className="text-xl font-black text-green-600">✓</span>
                        ) : (
                          <span className="text-xl font-black text-[#d6a84f]">{index + 1}</span>
                        )}
                      </div>

                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-[#9b6d1d] uppercase tracking-wide mb-0.5 truncate">{a.unitName}</p>
                        <h2 className="text-sm md:text-base font-black text-[#2d2d2d] truncate">{a.activityName}</h2>
                        {a.minimumScore != null && (
                          <p className="text-xs text-gray-400 mt-0.5">
                            Mínimo: <span className="font-semibold text-gray-600">{a.minimumScore}%</span>
                          </p>
                        )}
                        {done && a.previousScore != null && (
                          <span className={`text-xs font-semibold md:hidden ${a.previousApproved ? "text-green-600" : "text-red-500"}`}>
                      ...
                      <div className="flex-shrink-0 flex flex-col items-end gap-1">
                        {done ? (
                          <>
                            <span className="text-xs font-bold text-green-700 bg-green-100 px-2.5 md:px-3 py-1 md:py-1.5 rounded-full">Realizada</span>
                            {a.previousScore != null && (
                              <span className={`text-xs font-semibold hidden md:block ${a.previousApproved ? "text-green-600" : "text-red-500"}`}>
                                {a.previousScore}% — {a.previousApproved ? "Aprovado" : "Reprovado"}
                              </span>
                            )}
                          </>
                        ) : locked ? (
                          <span className="text-xs font-bold text-[#b0a080] bg-[#ede8da] px-2.5 md:px-3 py-1 md:py-1.5 rounded-full">Bloqueado</span>
                        ) : (
                          <span className="text-xs font-bold text-[#9b6d1d] bg-[#f4ead7] px-2.5 md:px-3 py-1 md:py-1.5 rounded-full">Disponível</span>
                        )}
                      </div>
                    </div>
                  );
